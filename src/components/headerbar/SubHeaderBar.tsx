...
export const SubHeaderBar = ({ centerContent, projectNumber, leftContent, rightContent }: SubHeaderBarProps) => {
    return (
        <header className="ads:flex ads:items-center ads:justify-between ads:w-full ads:h-14 px-8">
            ...
            <div className="ads:flex ads:flex-1 ads:justify-end">{projectNumber && <div className="ads:flex ads:items-center ads:w-fit ads:h-7 ads:sm:h-6 ads:py-0 ads:px-6 ads:rounded-full ads:bg-aibel-blue-background-tinted">{projectNumber}</div>}</div>
